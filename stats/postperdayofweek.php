<?php
	//include( "http://www.1142.org/session.php" );

	// establish connection to MySQL database or output error message.
	$link = mysql_connect ("1142.org", "org1142", "ultima");
	if (!mysql_select_db("org1142", $link)) echo mysql_errno().": ".mysql_error()."<BR>";
	
	// Local test db settings.
//	$link = mysql_connect ("localhost", "1142test");
//	if (!mysql_select_db("1142test", $link)) echo mysql_errno().": ".mysql_error()."<BR>";

	// Build the Query to get the post per hour

	$CommentsQuery = " select DAYNAME(dt_datePosted) as DayOfWeek, count(dt_DatePosted) as NUMBER";
	$CommentsQuery .= " from Comment";
	$CommentsQuery .= " group by DAYNAME(dt_datePosted) order by DAYNAME(dt_datePosted) ";
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML xmlns:v="urn:schemas-microsoft-com:vml">
<HEAD>
<TITLE>1142.org Graphs (Posts by Day of Week)</TITLE>
<META NAME="Author" CONTENT="Clayton Hannah">
<BODY MARGINWIDTH=0 MARGINHEIGHT=0>
<FONT FACE=VERDANA SIZE=4>
<CENTER>
<?PhP 
	echo "Post by Day of Week Breakdown for 1142.org<BR><BR><BR>";
	// Get comments
	$CommentsResultId = mysql_query ($CommentsQuery, $link);
	// output comments
	echo "<TABLE><TR VALIGN=BOTTOM>";
	while($CommentsResult = mysql_fetch_object($CommentsResultId) )
	{
		echo "<TD ALIGN=CENTER><FONT SIZE=1>".$CommentsResult->NUMBER."<BR><IMG SRC='bar.gif' BORDER=0 WIDTH=30 HEIGHT=".$CommentsResult->NUMBER."><BR>".$CommentsResult->DayOfWeek."</FONT></TD>";
	}
	echo "</TR></TABLE>";
?>
</CENTER>
</FONT>
</BODY>
</HTML>
<?php
	// close connection to MySQL Database
	mysql_close($link);
?>