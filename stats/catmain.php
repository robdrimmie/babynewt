...
//	}
	...
	if(Empty($CatId)&&Empty($CatName)) $CatId = 1;
	...
	$CommentsQuery = " SELECT Comment.i_CommentId, Comment.t_Comment, Comment.dt_DatePosted, Users.i_UID,	
	 ...
	 if(!Empty($CatId))	$CommentsQuery .= " AND Comment.i_CategoryId = ".$CatId;
	 else 	$CommentsQuery .= " AND Category.vc_Name = '".$CatName."'";
...
<?PhP 
	...
	echo "<form name=\"frmFilter\" action=\"catmain.php\" method=\"post\">";
	...
	while( 	$CommentsResult = mysqli_fetch_object($CommentsResultId) )
	{
		...
		echo "<form name=\"frmUpdateLastComment$CommentsResult->i_CommentId\" action=\"catmain.php\" method=\"post\">";
		echo "<input type=\"hidden\" name=\"CatName\" value=\"$CatName\">";
		echo "<input type=\"hidden\" name=\"CatId\" value=\"$CatId\">";
